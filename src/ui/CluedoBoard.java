...
public class CluedoBoard extends JComponent
{
	...
	public CluedoBoard( char[][] board )
	{
		...
		tileH = 20;
		tileW = 20;
		transform = AffineTransform.getScaleInstance(tileW / 200.0f, tileH / 200.0f);
		int i = 0, j = 0;
		char c;
		...
		for ( j = 0; j < height; ++j )
		{
			for ( i = 0; i < width; ++i )
			{
				...
				} else if ( isInRoom(c) )
				{
					...
					if ( c == 'd' )
					{
						char up =   j - 1 > 0      ? board[ i ][ j - 1 ] : board[ i ][ j ];
						char down = j + 1 < height ? board[ i ][ j + 1 ] : board[ i ][ j ];
						if ( up == 'S' )
					...
					{
						char left =  i - 1 > 0     ? board[ i - 1 ][ j ] : board[ i ][ j ];
						char right = j + 1 < width ? board[ i + 1 ][ j ] : board[ i ][ j ];
						if ( left == 'S' )
	...
	private void createPlaces( HashMap<Character, LinkedList<Point>> doors )
	{
		...
		HashMap<Character, Character> passages = new HashMap<Character, Character>();
		passages.put('K', 'U');
		passages.put('C', 'L');
		passages.put('U', 'K');
		passages.put('L', 'C');
		...
		for ( char ch: chs )
		{
			...
			leastx = Integer.MAX_VALUE; mostx = 0; leasty = Integer.MAX_VALUE; mosty = 0;
	...
	private boolean isNeighbour( Square s1, Square s2, int tileW, int tileH )
	{
		if ( ( s1.location.x == s2.location.x
				&& ( s1.location.y == s2.location.y + 1
				  || s1.location.y == s2.location.y - 1 ) )
		  || ( s1.location.y == s2.location.y
				&& ( s1.location.x == s2.location.x + 1
				  || s1.location.x == s2.location.x - 1 ) ) )
			return true;
		return false;
	}
			...
			HashMap<Character, LinkedList<Point>> map )
	{
		...
		if ( points == null )
			map.put(c, new LinkedList<Point>(Arrays.asList(new Point(x, y))));
		else
		{
	...
	protected void paintComponent( Graphics g )
	{
					...
					else
					{
						...
						if ( ch != 'd' )
						{
							...
							if ( j + 1 < height && testForWalls(test, i, j + 1) )
								g.fillRect(0, tileH - wallthickness, tileW, tileH); // wall
																					// below
						}
						if ( ch != 'D' )
						{
							...
							if ( i + 1 < width && testForWalls(test, i + 1, j) )
								g.fillRect(tileW - wallthickness, 0, tileW, tileH); // wall
																					// right
						}
					}
				// draw square tiles
		...
		g.drawString( Cards.KITCHEN.name(), (int)(b.width * 0.07f), (int)(b.height * 0.15f) );
		g.drawString(Cards.BALLROOM.name(), (int)(b.width * 0.44f), (int)(b.height * 0.20f));
		g.drawString(Cards.CONSERVATORY.name(), (int)(b.width * 0.79f), (int)(b.height * 0.15f));
		g.drawString(Cards.DININGROOM.name(), (int)(b.width * 0.07f), (int)(b.height * 0.52f));
		g.drawString(Cards.BILLIARDROOM.name(), (int)(b.width * 0.79f), (int)(b.height * 0.43f));
		g.drawString(Cards.LIBRARY.name(), (int)(b.width * 0.82f), (int)(b.height * 0.67f));
		g.drawString(Cards.LOUNGE.name(), (int)(b.width * 0.09f), (int)(b.height * 0.89f));
		g.drawString(Cards.HALL.name(), (int)(b.width * 0.47f), (int)(b.height * 0.87f));
		g.drawString(Cards.STUDY.name(), (int)(b.width * 0.82f), (int)(b.height * 0.94f));
